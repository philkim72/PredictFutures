...
combined2 = transform(combined1, BEAR = aaii[max.col(-abs(outer(DATE, netpos$DATE, `-`))), "BEAR"])
combined2 = transform(combined2, NEUTRAL = aaii[max.col(-abs(outer(DATE, netpos$DATE, `-`))), "NEUTRAL"])
combined2 = transform(combined2, BULL = aaii[max.col(-abs(outer(DATE, netpos$DATE, `-`))), "BULL"])
combined2$DATE <- format(combined2$DATE, "%Y-%m-%d")

combined2
```
```{r}
full_table <- merge(eod_returns1, combined2, by = "DATE")
#drop_cols <- c("Date")
...
```{r}
# This Pivot Longer function is not necessary
library(magrittr)
library(tidyr)
