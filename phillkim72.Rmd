...
netpos[which.min(abs(combined$DATE - netpos$DATE)),]

#combined$ES1_NET_POSITION <- netpos[which.min(abs(combined$DATE - netpos$DATE)),]
#combined
```
...
#netpos$Date <- format(netpos$DATE, "%Y-%m-%d")
#netpos <- netpos[,c(3,2)]
netpos
```
```{r}
full_table <- merge(eod_returns, combined, by = "Date")
drop_cols <- c("DATE")
