...
p <- ggplot(eod_returns15, aes(x=Day_of_Week, y=Return)) + geom_boxplot() +
  labs(title="Daily last 15 minutes return, by Day of the Week",
         x= "Day of Week")
p
```
...
p <- ggplot(eod_returns30, aes(x=Day_of_Week, y=Return)) + geom_boxplot() +
  labs(title="Daily last 30 minutes return, by Day of the Week",
         x= "Day of Week")
p
```
```{r, include=FALSE}
...
p <- ggplot(eod_returns60, aes(x=Day_of_Week, y=Return)) + geom_boxplot() +
  labs(title="Daily last 60 minutes return, by Day of the Week",
         x= "Day of Week")
p
```
...
p <- ggplot(eod_returns, aes(x=pre_t15_return, y=t15_return)) + geom_point() + geom_hline(yintercept=0) + geom_vline(xintercept=0) +
  labs(title="Daily last 15 minutes return, by Same Day returns (excluding last 15 minutes)",
         x= "Same Day Returns", y="Closing 15 minutes Returns")
p
```
...
p <- ggplot(full_table, aes(x=VIX, y=t15_return)) + geom_point() + geom_hline(yintercept=0) + geom_vline(xintercept=0) + coord_cartesian(xlim = c(10, 40)) +
  labs(title="Daily last 15 minutes return at levels of VIX",
         x= "VIX Volatility Index", y = "Closing Returns")
p
```
...
We also looked into Net Positioning, Fed Funds Rate and ISM Index, but neither showed much relationship with the closing returns.